...
public sealed class ChurnReportBuilder : IChurnReportBuilder
{
    ...
    public string BuildCsv(ChurnReport report, CodeChurnReportModel? policy = null)
    {
        var sb = new StringBuilder();
        sb.AppendLine("#,Component,Category,Repository,Branch,Activity,Work Items,Modified Files,Impacted Functionality,Test Use Cases,Build Status,Manual Test Cases");
        ...
        AppendPolicyCsv(sb, policy);
        return sb.ToString();
    }

    /// <summary>
    /// The footer is required in every output: a suppression nobody can see is indistinguishable from data
    /// that was never collected.
    /// </summary>
    private static void AppendPolicyCsv(StringBuilder sb, CodeChurnReportModel? policy)
    {
        if (policy is null) return;

        sb.AppendLine();
        sb.AppendLine("Feature,Work Items,Changes,Files");
        foreach (CodeChurnFeatureGroup group in policy.GroupsForDisplay)
        {
            sb.AppendLine(string.Join(",", new[]
            {
                Csv(group.FeatureId is { } id ? $"{id} {group.FeatureTitle}" : group.FeatureTitle),
                Csv(group.WorkItemCount.ToString(CultureInfo.InvariantCulture)),
                Csv(group.ChangeCount.ToString(CultureInfo.InvariantCulture)),
                Csv(group.FileCount.ToString(CultureInfo.InvariantCulture)),
            }));
        }

        sb.AppendLine();
        sb.AppendLine(Csv(policy.ExclusionFooter));
    }

    public string BuildHtml(ChurnReport report, CodeChurnReportModel? policy = null)
    {
        ...
        foreach (var h in new[] { "#", "Component", "Category", "Repository", "Branch", "Activity", "Work Items", "Modified Files", "Impacted Functionality", "Test Use Cases", "Build Status", "Manual Test Cases" })
        ...
        foreach (var r in report.Rows)
        {
            ...
            sb.Append($"<td style=\"padding:8px;min-width:190px;\">{ManualSuitesHtml(r)}</td>");
            sb.Append("</tr>");
            n++;
        }

        sb.Append("</tbody></table></div>");
        AppendPolicyHtml(sb, policy);
    ...
    private static void AppendPolicyHtml(StringBuilder sb, CodeChurnReportModel? policy)
    {
        if (policy is null) return;

        sb.Append("<div style=\"padding:16px 24px;border-top:1px solid #eee;\">");
        sb.Append("<div style=\"font-size:13px;font-weight:600;margin-bottom:8px;color:#1b1b28;\">Impacted Features</div>");
        sb.Append("<table style=\"border-collapse:collapse;font-size:12px;width:100%;\">");
        sb.Append("<thead><tr style=\"background:#faf9ff;text-align:left;\">"
            + "<th style=\"padding:6px;\">Feature</th><th style=\"padding:6px;\">Work items</th>"
            + "<th style=\"padding:6px;\">Changes</th><th style=\"padding:6px;\">Files</th></tr></thead><tbody>");

        foreach (CodeChurnFeatureGroup group in policy.GroupsForDisplay)
        {
            // Orphan bucket renders last and greyed, but always renders - a missing bucket reads as "none known".
            string style = group.IsOrphanBucket ? "color:#999;" : "";
            string label = group.FeatureId is { } id ? $"{id} \u00b7 {group.FeatureTitle}" : group.FeatureTitle;
            sb.Append($"<tr style=\"border-top:1px solid #eee;{style}\">");
            sb.Append($"<td style=\"padding:6px;\">{Enc(label)}</td>");
            sb.Append($"<td style=\"padding:6px;\">{group.WorkItemCount}</td>");
            sb.Append($"<td style=\"padding:6px;\">{group.ChangeCount}</td>");
            sb.Append($"<td style=\"padding:6px;\">{group.FileCount}</td>");
            sb.Append("</tr>");
        }

        sb.Append("</tbody></table>");
        sb.Append($"<div style=\"margin-top:10px;font-size:11px;color:#666;\">{Enc(policy.ExclusionFooter)}</div>");
        sb.Append("</div>");
    }
    ...
    private static string ManualSuitesText(SubsystemRow r) =>
        r.ManualSuites.Count == 0 ? "" : string.Join("; ", r.ManualSuites.Select(s =>
            string.IsNullOrWhiteSpace(s.Title) ? $"Test Case {s.SuiteId}" : $"Test Case {s.SuiteId} - {s.Title}"));

    private static string ManualSuitesHtml(SubsystemRow r)
    {
        if (r.ManualSuites.Count == 0) return "<span style=\"color:#999;\">\u2014</span>";
        return string.Join("<br>", r.ManualSuites.Select(s =>
        {
            var label = Enc(string.IsNullOrWhiteSpace(s.Title) ? $"Test Case {s.SuiteId}" : $"Test Case {s.SuiteId} - {s.Title}");
            return string.IsNullOrEmpty(s.Url) ? label : $"<a href=\"{Enc(s.Url)}\" style=\"color:#3b5bdb;text-decoration:none;\">{label}</a>";
        }));
    }
