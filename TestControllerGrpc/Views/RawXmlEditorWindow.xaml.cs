...
public partial class RawXmlEditorWindow : Window
{
    private readonly WatchItemXmlEditorViewModel _vm;
    private FoldingManager? _foldingManager;
    private XmlFoldingStrategy? _foldingStrategy;
    private CompletionWindow? _completionWindow;

    public RawXmlEditorWindow(WatchItemXmlEditorViewModel viewModel)
    {
        InitializeComponent();
        _vm = viewModel;
        DataContext = _vm;
    ...
    private void ShowElementCompletion()
    {
        ...
        data.Add(new XmlCompletionData("WatchItem", "Root element for a file watcher definition"));
        data.Add(new XmlCompletionData("Event", "Event trigger (Renamed/Created/Changed)"));
        ...
        data.Add(new XmlCompletionData("Ref", "Reference to a template"));
    ...
    private void ShowAttributeCompletion(string tagName)
    {
        ...
        {
            case "WatchItem":
                data.Add(new XmlCompletionData("Tag=\"\"", "WatchItem identifier tag"));
                data.Add(new XmlCompletionData("Path=\"\"", "Directory path to watch"));
                data.Add(new XmlCompletionData("Filter=\"\"", "File filter pattern (e.g. *.txt)"));
                break;
            case "Event":
                data.Add(new XmlCompletionData("Type=\"\"", "Renamed, Created, or Changed"));
            ...
            case "Action":
                ...
                data.Add(new XmlCompletionData("FailAndContinue=\"true\"", "Continue on failure"));
                data.Add(new XmlCompletionData("IsReboot=\"false\"", "Is this a reboot command"));
                ...
                data.Add(new XmlCompletionData("Attachment=\"\"", "Mail attachment path"));
    ...
    private void ShowValueCompletion(string attrName)
    {
        var values = attrName switch
        {
            ...
            "Type" when IsInsideTag("Event") => new[] { "Renamed", "Created", "Changed" },
    ...
    /// <summary>Checks if the caret is currently inside a specific tag name.</summary>
    private bool IsInsideTag(string tagName)
    {
        var offset = XmlEditor.CaretOffset;
        var lineText = GetCurrentLineTextUpToCaret(offset);
        var extractedTag = ExtractCurrentTagName(lineText);
        return string.Equals(extractedTag, tagName, StringComparison.Ordinal);
    }
    ...
    {
        if (e.PropertyName == nameof(WatchItemXmlEditorViewModel.XmlText))
        {
            if (XmlEditor.Text != _vm.XmlText)
                XmlEditor.Text = _vm.XmlText;
        }
        else if (e.PropertyName == nameof(WatchItemXmlEditorViewModel.IsPreviewVisible))
        {
